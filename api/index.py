...
try:
    ...
    try:
        ...
        # Create default admin user if needed (with error handling)
        try:
        ...
        @app.route('/')
        def error_page():
            return f"<h1>Configuration Error</h1><p>Please check your environment variables: {str(e)}</p>"
    ...
    # If there's an import error, create a minimal Flask app that shows the error
    ...
    @app.route('/')
    def error():
        return f"<h1>Import Error</h1><p>{str(e)}</p><p>Check your dependencies and environment variables.</p>"
